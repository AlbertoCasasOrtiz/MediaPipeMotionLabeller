...
class XmlGenerator:
    ...
    def generate(self):
        ...
        for movement in self.set_of_movements.get_movements():
            ...
            ElementTree.SubElement(xml_movement, 'movement-key-points').text = str(movement.get_key_points())
            ...
            for movement_error in movement.get_movement_errors():
                ...
                ElementTree.SubElement(xml_movement_error, 'movement-error-key-points').text = str(
                    movement_error.get_key_points())

        ElementTree.SubElement(xml_movement_set, 'movement-set-template').text = "\n" + json.dumps(
            self.set_of_movements.get_template(), indent=4) + "\n"
